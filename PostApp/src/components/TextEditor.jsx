...
// Destructure isClicked from props correctly
function TextEditor({ isClicked = false }) {
  ...
  }, [value, slugg, titlee, dispatch]);

  // FIX: Reset logic moved into useEffect to prevent infinite re-renders
  useEffect(() => {
    if (isClicked) {
      setslug("");
      settitle("");
  ...
  }, [isClicked]); // Only runs when the click status changes
